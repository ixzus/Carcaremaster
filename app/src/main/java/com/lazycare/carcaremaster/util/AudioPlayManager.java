...
import com.lazycare.carcaremaster.listener.VoiceBubbleListener;
...
public class AudioPlayManager {
	public static final String CACHE_VOICE_FILE_PATH = Environment
			.getExternalStorageDirectory() + "/donal/voice/";
	...
	public static AudioPlayManager getInstance(Context c,
	...
	private void downLoadPrepareFile(String url) {
		final File soundFile = new File(Config.BASE_PATH + "/voice/download/"
		...
		if (!soundFile.exists()) {
			if (!isDownloading) {
				...
				try {
					isDownloading = true;
					if (!(new File(Config.BASE_PATH + "/voice/download/"))
							.exists()) {
						(new File(Config.BASE_PATH + "/voice/download/"))
								.mkdirs();
					}
